...
object Texts {
  ...
  sealed abstract class Text {
    ...
    def isEmpty: Boolean = this match
      ...
      case Fluid(r) => r forall (_.isEmpty)
      case Vertical(r) => r.isEmpty
    ...
    def remaining(w: Int): Int = this match
      case Str(s) => w - lengthWithoutAnsi(s)
      case Fluid(Nil) => w
      case Fluid(last :: prev) =>
        val r = last.remaining(w)
        if r < 0 then r else Fluid(prev).remaining(r) 
      case Vertical(_) => -1
